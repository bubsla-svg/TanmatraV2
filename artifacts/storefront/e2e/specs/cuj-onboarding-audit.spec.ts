...
test.describe("OB-6 Onboarding Conversion Gate & Auth Audit", () => {
  ...
  test("commitment surfaces redirect cleanly to /login?next= when logged out", async ({ page }) => {
    const protectedPaths = ["/account", "/account/orders", "/account/preferences"];

    for (const path of protectedPaths) {
      await page.goto(path);
      // Must redirect or prompt clean authentication with preservation of intended destination
      await expect(page).toHaveURL(/login|next=/i);
    }
  });

  test("ServiceabilityBar unserviceable verdict renders notify form without blocking menu browsing", async ({ page }) => {
    const errors = collectErrors(page);
