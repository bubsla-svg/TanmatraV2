...
export function MobileBottomNav() {
  ...
  // One reason to slide away, one reason to inert: the bar is scrolled off.
  // It used to have a second — an "Account & Information" sheet this bar
  // opened — and that sheet is gone (2026-09-07). The Account tab is a Link to
  // /account like every other tab; the Company + Legal links it carried now
  // live on /account itself, outside AccountHub's auth gate so signed-out
  // visitors see them too.
  const barHidden = useScrollHide(false);
  ...
  // data-stitch on the bar root, not a page wrapper: this bar is painted dark
  // on every route, but it renders from app/layout.tsx — OUTSIDE the route
  // wrappers that carry the dark scope. Without it the tokens here resolve
  // their LIGHT arm on a near-black bar, and the selected tab is the worst
  // offender: text-gold gives the light-mode saffron at 3.37:1 while the three
  // unselected tabs sit at 7.10:1, so the current tab is the least legible
  // thing in the bar (10px labels — AA needs 4.5:1, and the currentColor icon
  // needs 1.4.11's 3:1). The scope flips --gold to the dark arm (8.53:1) and
  // --line to the 6% hairline instead of a bright light-mode rule.
  return (
    <nav
      data-stitch="dark"
