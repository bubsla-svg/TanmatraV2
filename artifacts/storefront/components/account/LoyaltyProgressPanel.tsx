    ...
    <div className="flex flex-col gap-3">
      ...
      <ul className="flex flex-col gap-3">
        {rows.map(({ sub, progress: p }) => (
          <li key={p.subscriptionId} className="rounded-xl border border-line bg-surface p-4">
            <p className="text-sm font-semibold text-ink">
              {sub.cadence} · {sub.mealsPerDelivery} meals / delivery
            </p>
            <p className="tabular mt-1 text-xs text-ink-muted">{p.deliveredCount} delivered</p>
            ...
            <p className="mt-0.5 text-xs text-ink-muted">
              {p.premiumUnlocked
                ...
                : `${p.deliveriesUntilPremium} more for a one-time loyalty bonus`}
            </p>
          </li>
        ))}
      </ul>
