  ...
  return (
    <button
      ...
      className={`relative w-full p-5 flex flex-col items-start text-left gap-3 rounded-3xl transition-all duration-200 active:scale-[0.98] ${
        isSelected
          ? "bg-[#171717] border-2 border-[#D4AF37] shadow-lg shadow-[#D4AF37]/5"
          : "bg-[#171717] border-2 border-white/5 hover:border-white/10 hover:bg-[#1C1C1C]"
      } ${className}`}
    >
      {isSelected && (
      ...
      {icon && <div className={`p-2 rounded-xl ${isSelected ? 'bg-[#D4AF37]/10 text-[#D4AF37]' : 'bg-white/5 text-[#A3A3A3]'}`}>{icon}</div>}
      <div>
        <h4 className={`text-base font-semibold tracking-tight mb-1 ${isSelected ? 'text-[#F5F5F4]' : 'text-[#F5F5F4]'}`}>
          {title}
        </h4>
        {description && (
