...
export const InlineError: React.FC<InlineErrorProps> = ({ message, className = "" }) => {
  ...
  return (
    <div className={`flex items-start gap-2 text-[#B0655A] bg-[#B0655A]/10 p-3 rounded-lg border border-[#B0655A]/20 ${className}`}>
...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  return (
    <div className={`flex flex-col items-center justify-center text-center p-8 max-w-md mx-auto ${className}`}>
      {imageUrl && (
        <div className="w-48 h-48 mb-6 rounded-full overflow-hidden bg-[#171717] border border-white/5 relative">
          <img src={imageUrl} alt="" className="w-full h-full object-cover opacity-80" />
        </div>
      )}
      <h3 className="text-xl font-bold text-[#F5F5F4] mb-2">{title}</h3>
