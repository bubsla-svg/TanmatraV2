...
const numInput = "rounded-lg border border-line bg-surface px-2 py-1.5 text-sm font-normal text-ink";
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-2 rounded-lg border border-line px-2 py-1">
        <button type="button" aria-label={`Decrease ${label}`} onClick={() => onChange(value - 1)} className="text-ink-muted">&minus;</button>
        <span className="tabular w-8 text-center text-sm text-ink">{value}</span>
        <button type="button" aria-label={`Increase ${label}`} onClick={() => onChange(value + 1)} className="text-ink-muted">+</button>
      </div>
    </div>
  );
}
...
export function DietProfileForm({ initial, busy, saved, error, onSubmit }: {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div>
        <p className="text-sm font-medium text-ink">Allergens to avoid</p>
            ...
            return (
              <li key={a}>
                <button type="button" aria-pressed={on}
                  ...
                  className={`rounded-full px-3 py-1 text-xs font-medium capitalize ${on ? "border border-[var(--gold)] bg-[color-mix(in_srgb,var(--gold)_12%,transparent)] text-gold-text" : "border border-line text-ink-muted hover:border-line-strong"}`}>
                  {allergenLabel(a)}
                </button>
              </li>
            );
      ...
      <div className="grid grid-cols-2 gap-3">
        <label className="flex flex-col gap-1 text-sm font-medium text-ink">Calorie floor
        ...
        <label className="flex flex-col gap-1 text-sm font-medium text-ink">Calorie ceiling
      ...
      <label className="flex flex-col gap-1 text-sm font-medium text-ink">Notes for the kitchen
        <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} maxLength={1000} placeholder="Anything else the planner should respect…" className={numInput} /></label>

      {error && <p role="alert" className="text-xs font-medium text-[var(--danger)]">{error}</p>}
      <div className="flex items-center gap-3">
        <button type="button" disabled={busy} onClick={submit} className="self-start rounded-xl bg-gold px-5 py-3 text-sm font-semibold text-[var(--gold-ink)] disabled:opacity-40">
