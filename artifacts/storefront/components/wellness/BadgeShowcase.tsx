    ...
    <div className="space-y-8">
      ...
      <div className="bg-surface rounded-3xl p-6 sm:p-8 border border-line shadow-xl space-y-6">
        <div className="flex items-center gap-3 border-b border-line pb-4">
          ...
          <div>
            <h3 className="text-lg font-black text-ink font-heading">Badge Showcase</h3>
          ...
          {BADGES_CATALOG.map(b => (
            <div
              ...
              className={`p-3.5 rounded-2xl border text-center space-y-1.5 transition-all ${
                b.unlocked 
                  ? 'bg-surface-subtle border-[var(--gold)]/40 shadow-sm' 
                  : 'bg-surface-subtle border-line opacity-50 grayscale'
              }`}
            >
      ...
      <div className="bg-gradient-to-br from-[var(--gold)]/15 to-[var(--surface)] text-ink p-6 rounded-3xl border border-[var(--gold)]/40 shadow-xl space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Gift className="w-5 h-5 text-[var(--gold-text)]" />
            <h4 className="text-sm font-bold font-heading text-ink">Family Milestone Reward</h4>
          </div>
          <span className="text-xs font-black text-[var(--gold-text)]">1,450 / 2,000 Pts</span>
        </div>
