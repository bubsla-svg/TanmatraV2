    ...
    <div className="bg-surface rounded-2xl p-6 sm:p-8 border border-line space-y-6">
      <div className="flex items-center justify-between border-b border-line pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-display text-lg font-semibold leading-tight text-primary">Circadian Intermittent Fasting</h3>
            <span className="text-xs text-ink-muted font-medium">Metabolic Stage & Digestive Reset</span>
          </div>
        </div>

        <span className="px-2.5 py-1 rounded-full bg-gold/10 text-primary text-xs font-black border border-[var(--gold)]/20">
      ...
      <div className="space-y-2">
        <label className="text-[10px] font-bold uppercase tracking-[.16em] text-ink-muted block">Select Fasting Protocol:</label>
        <div className="grid grid-cols-2 gap-2">
          {FASTING_MODES.map(mode => (
            ...
            >
              ...
              <span className="font-data text-2xs opacity-80 block mt-1">{mode.fastingHours}h Fast • {mode.eatingHours}h Eat</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="bg-surface text-ink p-6 rounded-2xl border border-line text-center space-y-4 relative overflow-hidden">
        <div className="space-y-1">
          <span className="text-[11px] font-bold uppercase tracking-[.18em] text-accent flex items-center justify-center gap-1">
            <Flame className="w-3.5 h-3.5" /> Fasting Timer Clock
          </span>
          <strong className="font-data text-4xl sm:text-5xl font-bold tracking-tight text-primary block">
      ...
      <div className="p-4 rounded-2xl bg-secondary border border-line space-y-2">
        <div className="flex items-center gap-2">
      ...
      <div className="p-4 rounded-2xl bg-sage/10 border border-[var(--sage)]/30 flex items-start gap-3">
        ...
        <div>
          <strong className="text-xs font-bold text-sage-text block font-display">ICMR Electrolyte Clinical Tip</strong>
