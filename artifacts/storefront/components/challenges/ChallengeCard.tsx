 ...
 *  ended are neutral so they never compete visually with the sage Featured badge. */
const STATUS_PILL_CLASS: Record<string, string> = {
  live: "bg-sage-soft text-sage-text",
  soon: "bg-secondary text-ink-muted",
  ended: "bg-secondary text-ink-muted",
};
  ...
  return (
    ...
    >
      <div className="relative aspect-[16/9] w-full overflow-hidden bg-secondary">
        ...
        <span
          className={`absolute left-3 top-3 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.12em] ${STATUS_PILL_CLASS[status]}`}
        >
          {STATUS_LABEL[status]}
        </span>
        {c.featured > 0 && (
          <span className="absolute right-3 top-3 rounded-full bg-sage px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.12em] text-sage-foreground">
            Featured
          </span>
        )}
      </div>
      <div className="flex flex-1 flex-col p-5">
        <h3 className="font-display text-lg font-semibold leading-tight text-primary">{c.title}</h3>
        <p className="mt-1.5 line-clamp-2 text-sm leading-relaxed text-ink-muted">{c.tagline}</p>
        <div className="mt-3 flex flex-wrap gap-1.5">
          {c.goalTags.slice(0, 3).map((t) => (
            <span key={t} className="rounded-full bg-secondary px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.12em] text-ink-muted">{t}</span>
          ))}
        </div>
        <dl className="font-data mt-4 flex gap-3 border-t border-line pt-3 text-2xs font-medium text-ink-faint">
