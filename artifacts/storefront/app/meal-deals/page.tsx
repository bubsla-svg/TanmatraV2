      ...
      <div className="px-gutter pt-6 flex flex-col gap-6">
        {bundles.map((bundle) => (
          <div key={bundle.id} className="rounded-3xl border border-line bg-surface p-5 flex flex-col gap-4 shadow-sm">
            ...
            <button className="w-full mt-2 rounded-full bg-yellow-500 px-4 py-3 text-sm font-bold tracking-tight text-slate-900 hover:bg-yellow-500/90 transition-transform active:scale-[0.98]">
              Select Bundle
            </button>
          </div>
        ))}
