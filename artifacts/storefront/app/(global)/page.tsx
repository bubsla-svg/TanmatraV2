...
export default async function HomePage() {
  ...
  return (
    ...
    >
      {/* A <main>, not a <main>: app/layout.tsx already opens `<main id="main">`
      ...
      <main className="flex flex-col gap-10 sm:gap-16 lg:gap-20">
        ...
        <section className="px-gutter">
          <main className="flex justify-between items-end mb-6">
          ...
          <main className="flex gap-4 overflow-x-auto snap-x no-scrollbar pb-4 -mx-gutter px-gutter">
            {featuredDishes.map((dish) => (
              <main key={dish.id} className="flex-none w-[280px] snap-start">
        ...
        <section className="px-gutter py-8">
          <main className="flex gap-3 overflow-x-auto snap-x no-scrollbar -mx-gutter px-gutter">
        ...
        <section className="px-gutter py-6">
          ...
          <main className="flex gap-4 overflow-x-auto snap-x no-scrollbar pb-4 -mx-gutter px-gutter">
            {['Metabolic Reset', 'Performance Protocol'].map((plan, i) => (
              <main key={i} className="flex-none w-[300px] snap-start rounded-3xl border border-line bg-surface p-6">
                <main className="font-label-caps text-label-caps text-primary mb-2">SUBSCRIPTION</main>
        ...
        <main id="proofs" className="flex flex-col gap-10 sm:gap-16">
        ...
        <section className="px-gutter pb-8">
          <main className="rounded-2xl border border-primary/20 bg-primary/5 p-6 relative overflow-hidden">
            <main className="absolute top-0 right-0 w-32 h-32 bg-primary/10 rounded-full blur-3xl -mr-10 -mt-10 pointer-events-none"></main>
            <main className="flex gap-3 items-center mb-4">
