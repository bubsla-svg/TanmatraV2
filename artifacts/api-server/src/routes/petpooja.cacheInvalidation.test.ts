...
test("push-menu invalidates the catalog cache — a POS price push is visible immediately", async () => {
  const warm = await getMergedCatalog();
  ...
  assert.equal(
    ...
    25_000,
    "expected the pushed price immediately — a stale cache here means the storefront shows one price while the POS believes another",
  );

  // Restore.
  await db.update(menuItemsTable).set({ pricePaise: 10_000 }).where(eq(menuItemsTable.slug, SLUG));
  _resetMenuCatalogCacheForTests();
});
