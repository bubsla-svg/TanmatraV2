...
const { default: router } = await import("./partnerLeads");


test("partnerLeads.ts gated endpoints return 403 when unauthenticated", async () => {
  const app = express();
  ...
  try {
    {
      const res = await fetch(`${base}/partners/leads`, { method: "GET" });
      assert.equal(res.status, 403, "GET /partners/leads should be gated");
    }
  } finally {
    server.close();
  }
