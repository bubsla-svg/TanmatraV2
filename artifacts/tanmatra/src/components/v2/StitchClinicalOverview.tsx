...
export default function StitchClinicalMealCard({
  ...
  return (
    <div
      className="overflow-hidden flex flex-col rounded-lg border transition-all duration-200 hover:border-nn-primary/40"
      style={{
    ...
    >
      ...
      <div className="relative aspect-[16/10] w-full bg-nn-bg overflow-hidden">
        <img
          src={image}
          alt={name}
          loading="lazy"
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-nn-bg via-transparent to-black/30" />

        {/* Veg/Non-Veg dot */}
        <span
          className="absolute top-2.5 left-2.5 z-10 w-4 h-4 rounded border-2 border-nn-bg flex items-center justify-center"
          style={{ backgroundColor: isVeg ? "var(--color-clinical-sage)" : "var(--color-error)" }}
          aria-label={isVeg ? "Vegetarian" : "Non-Vegetarian"}
        >
          <span className="w-1.5 h-1.5 rounded-full bg-nn-bg" />
        </span>
      ...
      <div className="p-3 flex flex-col gap-2.5">
        ...
        <div
          className="flex items-center justify-between px-2.5 py-1.5 rounded bg-nn-bg border border-white/5 text-[11px]"
        ...
        >
          ...
          <span className="text-nn-primary font-bold">{proteinG}g P</span>
          <span className="text-white/30">|</span>
        ...
        <div className="flex items-center justify-between pt-1">
          <div className="flex items-baseline gap-1.5">
            <span
              className="text-lg font-extrabold text-nn-primary"
              style={{ fontFamily: STITCH_DESIGN_SYSTEM.typography.dataFont }}
            >
              ₹{priceRupees}
            </span>
          ...
          <button
            ...
            className="px-3.5 py-2 rounded font-extrabold text-xs tracking-wider uppercase bg-nn-primary text-stone-900 hover:bg-nn-primary/90 active:scale-95 transition-all flex items-center gap-1 touch-target-48"
          >
