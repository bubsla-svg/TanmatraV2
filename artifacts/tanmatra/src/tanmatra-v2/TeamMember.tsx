...
const ACCENT: Record<string, { text: string; bg: string; border: string }> = {
  ...
  blue: { text: "var(--color-nn-tertiary)", bg: "color-mix(in srgb, var(--color-nn-tertiary) 12%, transparent)", border: "color-mix(in srgb, var(--color-nn-tertiary) 55%, transparent)" },
};
            ...
            <div className="padx mt20">
              ...
              <div className="prodgrid">
                {ownedDishes.map((d: any) => (
                  <Link key={d.id} to={`/dish/${d.slug}`} className="rounded-xl bg-[var(--tnm-surface-ink-2)] border border-white/[0.08] shadow-[0_4px_16px_color-mix(in_srgb,black_20%,transparent)] p-0 overflow-hidden pointer" style={{ display: "block" }}>
                    <div
                      className="plc"
