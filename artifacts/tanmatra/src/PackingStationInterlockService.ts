...
export class PackingStationInterlockService {
  ...
  public async supervisorOverrideReset(
    ...
    const requiredPin = process.env.SUPERVISOR_OVERRIDE_PIN;
    if (!requiredPin) {
      throw new Error('Missing SUPERVISOR_OVERRIDE_PIN environment variable. Failing closed.');
    }

    if (supervisorPin !== requiredPin) {
