...
export class CryptographicWormLogger {
  ...
  constructor(
    ...
    const secret = process.env.WORM_AUDIT_HMAC_KEY;
    if (!secret) {
      throw new Error('Missing WORM_AUDIT_HMAC_KEY environment variable. Failing closed.');
    }
  ...
  public async logEvent(
    ...
    const eventId = `aud_${Date.now()}_${(crypto.randomBytes(4) as any).toString('hex')}`;
