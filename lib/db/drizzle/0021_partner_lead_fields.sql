...
-- with {"error":"lead capture failed"} against a database still on the old shape.
--
-- All three statements are metadata-only in Postgres: widening a varchar does
-- not rewrite the table, and both new columns are nullable with no default. Safe
-- to apply while the old server is still serving.
