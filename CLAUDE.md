...
The storefront runs its own suite — 60 files, 318 tests, all in `artifacts/storefront/lib/`
and all DB- and network-free (every API client takes an injectable `fetchImpl`):
```bash
pnpm --filter @workspace/storefront run test     # all 318, ~8s
...
node --test --import tsx "../storefront/lib/**/*.test.ts"   # 318 tests
```
