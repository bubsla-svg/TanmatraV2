...
All three M-5 dependencies logged above are now resolved, not merely deferred:

1. **`/menu/ranked` (and `/menu/public`) availability filter — resolved as "no filter, by design," not a gap.** The manual's original §7.6 acceptance ("disabled dishes render nowhere") is superseded by **D-19** (§10 — merged, PR #50): the shipped, tested product decision is "show a paused (`isAvailable:false`) dish with a disabled 'Back soon' Add button," not hide it. `components/cart/AddToCart.tsx`'s `AddToCartResolved` implements exactly this, and `e2e/specs/availability.spec.ts` asserts the live menu carries at least one visible paused dish. Filtering `isAvailable` out of either menu-listing route would silently revert a merged decision. **`archived`** (a *different*, unrelated M-3 CUT/MERGE/DELIST state — a dish no longer exists at all, not merely paused) *is* excluded from both listing routes, at the route layer rather than inside `getMergedCatalog()` — that resolver also backs checkout/subscriptions/loyalty, which must keep resolving a dish a customer already committed to even after it's archived.
2. **Two storefront ₹ literals (§8) — deliberately NOT fixed here.** Both are marketing/landing copy (`Section03B2BEnterprise.tsx`, `app/(focus)/trial/page.tsx`), not the `/menu` grid this branch touches, and the fix is "fold into the standing server-token program" per the original finding — a distinct, larger initiative (server-driven marketing-copy tokens) out of scope for a menu-display PR. Left as a follow-up.
3. **Veg tri-state render — implemented.** `DishData.vegClass` (`"veg"|"non-veg"|"egg"`, M-2 column) renders on every `DishCard` (both variants) via a new `VegMark`/`resolveVegClass` — falls back to the binary `isVeg` when `vegClass` is unset, so ungoverned dishes still render correctly.

Also implemented, beyond the three logged dependencies (full M-5 scope): thirteen fixed §5 sections with an ungoverned "More dishes" trailing bucket (`lib/menuSections.ts`, storefront-local — not imported from `@workspace/menu-catalog` to avoid bundling the ~5000-line static seed into client JS); in-section `sort_rank` ordering; a merchandising badge chip (`DishData.badge`); a macros chip (was bare text); personalization re-ranks within a section via the existing CSS-order mechanism but never hides (unchanged — diet-chip/filter/search remain the only hiding mechanism, all user-initiated); section 13 ("Off the Wok") is excluded from the personalization re-rank entirely (`NO_PERSONALIZATION_BOOST_SECTION`) — interpreted as freezing its order symmetrically (no promotion *or* demotion), the simplest reading of "excluded from ranking boosts" that doesn't invent an asymmetric boost/sink split the manual excerpt doesn't specify.

**Not implemented — cannot be, not a gap in this PR:** the manual's "Experience Contract — ten laws" review gate and formal "flipbook" process it cites for M-5. Exhaustive repo grep (including the root `CLAUDE.md`) finds no such document anywhere — only scattered, unrelated citations to individual law numbers (5, 8, 9.5, 10; one of those citations itself notes its own source is missing). `e2e/specs/stitch-runtime/menu-flipbook.spec.ts` captures evidence screenshots for every meaningful `/menu` state using this repo's actual, existing verification pattern instead of a document that doesn't exist. This is an owner action item, not something fixable from inside the repo.
