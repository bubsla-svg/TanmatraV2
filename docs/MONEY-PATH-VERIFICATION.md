...
[ ] Sponsored zero-charge: one confirmed order, one release, one ticket (evidence
    rewritten to a real join, §10 — pending this session's CI run, not yet green)
...
[ ] Browser-controlled identifiers cannot prove financial settlement (fix + tests
    written, §10 — pending this session's CI run, not yet green)
```
...
| `subscriptionDeliveries.orderId` | `subscription_deliveries` | integer, nullable | `createOrderForNewSubscription`, in the same transaction as the subscription and order (`UPDATE ... WHERE id = firstDelivery.id`) | **No** — no route accepts this column from a request body; confirmed by a repo-wide grep of every writer | Written once per delivery row | not declared unique, but written exactly once by exactly one function | **new** settlement evidence (§ below); `payments.ts registerAutopayMandate`; `chargeMandate.ts` |
...
### Adversarial tests added (`lib/reconciliationScheduler.test.ts`, `routes/loyalty.checkout.test.ts`)
